...
class ChatListViewModelTest {
    ...
    fun `starts loading with no items`() =
        runTest(dispatcher) {
            val viewModel = ChatListViewModel(FakeChatRepository(clock), clock)
    ...
    fun `carries title, snippet and id from the repository`() =
        runTest(dispatcher) {
            val repository = FakeChatRepository(clock)
            val id = repository.send(null, CHAT_TITLE)
            repository.emitDelta(id, SNIPPET)
            repository.completeTurn(id)
            val viewModel = ChatListViewModel(repository, clock)
            collecting(viewModel)
            advanceUntilIdle()
            ...
            assertEquals(CHAT_TITLE, item.title)
     ...
     */
    private fun TestScope.collecting(viewModel: ChatListViewModel) {
