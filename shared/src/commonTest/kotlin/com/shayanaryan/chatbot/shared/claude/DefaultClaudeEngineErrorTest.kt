...
class DefaultClaudeEngineErrorTest {
    private val request =
        ClaudeMessageRequest(
            messages = listOf(ClaudeMessage(Role.User, listOf(ContentBlock.Text(USER_MESSAGE)))),
        )

    private suspend fun errorFor(status: Int): ApiError {
        val events =
            testChatEngine {
                respondError(
        ...
        return assertIs<ClaudeStreamEvent.Failed>(events.single()).error
    }

    @Test
    fun `maps authentication statuses`() =
        runTest {
            assertEquals(ApiError.Authentication, errorFor(401))
            assertEquals(ApiError.Authentication, errorFor(403))
        }

    @Test
    fun `maps client server and overload statuses`() =
        runTest {
            assertEquals(ApiError.InvalidRequest, errorFor(400))
            assertEquals(ApiError.InvalidRequest, errorFor(404))
            assertEquals(ApiError.InvalidRequest, errorFor(413))
            assertEquals(ApiError.Server, errorFor(500))
            assertEquals(ApiError.Overloaded, errorFor(529))
        }

    @Test
    fun `maps timeout statuses`() =
        runTest {
            assertEquals(ApiError.Timeout, errorFor(408))
            assertEquals(ApiError.Timeout, errorFor(504))
        }
    ...
    fun `parses retry after on a rate limit`() =
        runTest {
            ...
            assertEquals(
                ...
                assertIs<ClaudeStreamEvent.Failed>(events.single()).error,
            )
        }
    ...
    fun `rate limit without a usable header carries no hint`() =
        runTest {
            assertEquals(ApiError.RateLimited(null), errorFor(429))
        }
    ...
    fun `a mid stream error event terminates the flow`() =
        runTest {
            ...
            assertEquals(ClaudeStreamEvent.Delta(PARTIAL_REPLY), events.first())
            assertEquals(ClaudeStreamEvent.Failed(ApiError.Overloaded), events.last())
    ...
    fun `a malformed frame is unexpected`() =
        runTest {
            ...
            assertEquals(ClaudeStreamEvent.Failed(ApiError.Unexpected), events.single())
        }
    ...
    fun `a truncated stream is unexpected`() =
        runTest {
            ...
            assertEquals(ClaudeStreamEvent.Delta(PARTIAL_REPLY), events.first())
            assertEquals(ClaudeStreamEvent.Failed(ApiError.Unexpected), events.last())
        }
    ...
    fun `lost connectivity is a network error`() =
        runTest {
            ...
            assertEquals(ClaudeStreamEvent.Failed(ApiError.Network), events.single())
        }
    ...
    fun `a byte gap stall is a timeout`() =
        runTest {
            ...
            assertEquals(ClaudeStreamEvent.Failed(ApiError.Timeout), events.single())
        }
}
