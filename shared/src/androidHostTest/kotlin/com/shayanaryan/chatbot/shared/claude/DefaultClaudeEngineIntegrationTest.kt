...
class DefaultClaudeEngineIntegrationTest {
    ...
    fun `streams a real turn end to end`() =
        runTest(timeout = 2.minutes) {
            ...
            val completed = assertIs<ClaudeStreamEvent.Completed>(events.last())
            ...
            val text =
                events.filterIsInstance<ClaudeStreamEvent.Delta>().joinToString(
                    "",
                ) { it.text }
            assertTrue(
        ...
        runTest(timeout = 2.minutes) {
            ...
            val request =
                ClaudeMessageRequest(
                    messages =
                        listOf(
                            ClaudeMessage(Role.User, listOf(ContentBlock.Text(USER_MESSAGE))),
                        ),
                )
